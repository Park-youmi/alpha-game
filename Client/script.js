const scriptUrl = 'https://script.google.com/macros/s/AKfycbxKCsaLn1CyxpVlZBh6M8j0YPrnROVzsmSH1X30_SVuJan1AhfjT24Gy8SIMHiZJuMu/exec'; // Google Apps Script URL

class Animal {
    constructor(name, food, growthStages, buffs, images) {
        this.name = name;
        this.food = food; // 각 동물의 먹이
        this.happiness = 1; // 초기 행복도를 1로 설정
...
function feedAnimal(index) {
    ...
    // 게임 종료 여부 체크하고, 게임이 끝났다면 간식 옵션을 비활성화
    if (selectedAnimal.isGameOver()) {
...
function endGame() {
    ...
    selectedAnimal.happiness = 1; // 행복도 초기화
